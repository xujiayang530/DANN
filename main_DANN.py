...
def get_dataset1(test_id,session): ## dataloading function, you should modify this function according to your environment setting.
    # path='F:\\zhourushuang\\transfer_learning\\feature_for_net_session'+str(session)+'_LDS_de'
    path = 'E:/数据集汇总/SEED/PR-PL-main/feature_LDS_de/'
    os.chdir(path)
#    path='F:\\zhourushuang\\transfer_learning\\feature_for_net_session1_LDS_de'
    feature_list=[]
    label_list=[]
    ...
    for info in os.listdir(path):
        ...
        feature = scio.loadmat(info_)['feature']
        label = scio.loadmat(info_)['label']+1
...
def train_model(loader_train, loader_test,model,dann_loss, optimizer,scheduler,hidden_4,epoch,batch_size,parameter,threshold_update=True): ## model training 
    ...
    for i in range(T):  #训练集和测试集是相同大小的，训练集来自于多个源域并打乱
        ...
        # 相似度矩阵的参考，源域因为有标签，因此参考就是标签，目标域因为没有标签，因此用阈值
        # estimated_sim_truth,estimated_sim_truth_target = get_generated_targets(model,x_s,x_t,labels_s)#参考的相似度矩阵
        ...
        _,feature_source_f,feature_target_f,feature_source_f_softmax,feature_target_f_softmax = model(x_s,x_t,labels_s)


        CEL = nn.NLLLoss()
        L_s = torch.argmax(labels_s, dim=1)
        cls_loss = CEL(feature_source_f_softmax, L_s.long()).cuda()
        ...
        # cluster_loss=torch.sum(indicator*bce_loss_target)/nb_selected#因为有些相似度在上下阈值之间，因此要排除这些影响
        # regularization  
...
def train_and_test_GAN(test_id,max_iter,parameter,session,threshold_update=True): ## pipeline for PR-PL model
    ...
    target_set,source_set=get_dataset1(test_id,session)
    ...
    model=Domain_adaption_model_withoutproto_withoutpair(hidden_1,hidden_2,hidden_3,hidden_4,num_of_class,low_rank,max_iter,upper_threshold,lower_threshold).cuda(0)
...
print('mean acc: ', np.mean(result_list['best_acc_mat']))
print('std acc: ', np.std(result_list['best_acc_mat']))
